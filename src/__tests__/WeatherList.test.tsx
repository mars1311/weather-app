...
describe("WeatherList", () => {
  const mockCities = [
    { id: 1, name: "New York", temp: 10, condition: "Rainy", lastUpdated: "10:00 AM" },
    { id: 2, name: "Tokyo", temp: 18, condition: "Clear", lastUpdated: "11:00 AM" },
  ] as any;

  test("renders Empty component when cities list is empty", () => {
    (useAppSelector as jest.Mock).mockReturnValue(null);
    render(<WeatherList cities={[]} onRefresh={jest.fn()} onDelete={jest.fn()} />);
  ...
  test("renders the correct number of weather cards", () => {
    (useAppSelector as jest.Mock).mockReturnValue(null);
    render(<WeatherList cities={mockCities} onRefresh={jest.fn()} onDelete={jest.fn()} />);
